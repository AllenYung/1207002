...
export default {
    ...
    methods: {
        async loadContent() {
            try {
                const response = await fetch("src/components/Java55.txt"); // 替換成你的文件路徑
                if (response.ok) {
                    this.conn = await response.text();
                } else {
        ...
        ...mapActions(counter,["zero","p4close"]),
    },
...
<template>
    <div class="borderr">
        <div class="top">
            <div class="imgL"></div>
            <span>Java</span>
            <div class="imgR" @click="p4close()"></div>
        </div>
        <div class="topM">
        ...
        <div class="inside">
            <pre class="inin" v-html="conn"></pre>
        </div>
    </div>
</template>
