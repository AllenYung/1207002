...
export default {
    ...
    methods: {
        async loadContent() {
            try {
                const response = await fetch("src/components/html111.txt"); // 替換成你的文件路徑
                if (response.ok) {
        ...
        ...mapActions(counter,["zero","p3close"])
    },
};
</script>
...
<template>
    <div class="borderr">
        <div class="top">
            <div class="imgL"></div>
            <span>HTML</span>
            <div class="imgR" @click="p3close()"></div>
        </div>
        <div class="topM">
        ...
        <div class="inside">
            <pre class="inin" v-html="content"></pre>
        </div>
    </div>
</template>
